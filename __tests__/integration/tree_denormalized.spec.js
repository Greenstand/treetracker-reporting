...
describe('tree_denormalized', () => {
    ...
    describe('Trees GET', () => {
        ...
        it(`Should raise validation error with error code 422 -- 'tree_uuid' query parameter should be a uuid`, async () => {
          const result = await request(app).get('/tree')
            .query({
                tree_uuid: 'tree_uuid',
            })
            .expect(422);
        ...
        it(`Should raise validation error with error code 422 -- 'planting_organization_uuid' query parameter should be a uuid  `, async () => {
            const result = await request(app).get('/tree')
              ...
              .expect(422);
              expect(result.body.message).to.eql('"planting_organization_uuid" must be a valid GUID');
        });
        ...
        it(`Should raise validation error with error code 422 -- 'sort_by' query parameter should be one of the allowed values  `, async () => {
            const result = await request(app).get('/tree')
              ...
              .expect(422);
              expect(result.body.message).to.eql('"sort_by" must be one of [tree_uuid, tree_created_at, planter_first_name, planter_last_name, planter_identifier, created_at, lat, lon, note, planting_organization_uuid, planting_organization_name, species]');
        });
        ...
        it('Should get trees statistics successfully', async () => {
            const result = await request(app).get('/tree/statistics')
              .expect(200);
            ...
            expect(result.body.top_planters).to.have.keys(['average', 'top_planters']);
        ...
        it(`Should raise validation error with error code 422 -- 'card_title' query parameter should be one of the select values `, async () => {
            const result = await request(app).get('/tree/statistics/card')
              .query({
                    card_title: 'card_title',
              })
              .expect(422);
              expect(result.body.message).to.eql('"card_title" must be one of [planters, species, trees, unverified_trees, top_planters, trees_per_planters, catchments, gender_details]');
        });
        ...
        it(`Should raise validation error with error code 422 -- 'tree_created_start_date' query parameter should be a date  `, async () => {
            const result = await request(app).get('/tree/statistics/card')
              ...
              .expect(422);
              expect(result.body.message).to.eql('"tree_created_start_date" must be in ISO 8601 date format');
        });
        ...
        it(`Should raise validation error with error code 422 -- 'tree_created_end_date' query parameter should be a date  `, async () => {
            const result = await request(app).get('/tree/statistics/card')
              ...
              .expect(422);
              expect(result.body.message).to.eql('"tree_created_end_date" must be in ISO 8601 date format');
        });
